...
ModelAnimatorInstancing::ModelAnimatorInstancing(string name)
    : ModelAnimator(name)
{
    SetShader(L"Shaders/Model/ModelAnimationInstancing.hlsl");
    // 인스턴싱 버퍼 생성
...
void ModelAnimatorInstancing::Render()
{
    ...
    frameInstancingBuffer->SetVertexShaderBuffer(4);
    DC->VSSetShaderResources(0, 1, &srv);

    for (ModelMesh* mesh : meshes)
        mesh->RenderInstanced(drawCount);

    //meshes[0]->GetMaterial()->GetMaterialData().emissive = COLOR_RED;
}
