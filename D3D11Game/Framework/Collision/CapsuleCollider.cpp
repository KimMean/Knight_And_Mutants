...
{
    Vector3 direction = transform->Up();    // 방향
    // 꼭지점
    Vector3 pa = transform->GlobalPos() - direction * Height() * 0.5f;
    Vector3 pb = transform->GlobalPos() + direction * Height() * 0.5f;
    // 원 사이즈
    float r = Radius();
...
bool CapsuleCollider::IsSphereCollision(SphereCollider* collider)
{
    ...
    Vector3 P = collider->transform->GlobalPos();
...
{
    Vector3 aDirection = transform->Up();
    Vector3 aA = transform->GlobalPos() - aDirection * Height() * 0.5f;
    Vector3 aB = transform->GlobalPos() + aDirection * Height() * 0.5f;

    Vector3 bDirection = collider->transform->Up();
    Vector3 bA = collider->transform->GlobalPos() - bDirection * collider->Height() * 0.5f;
    Vector3 bB = collider->transform->GlobalPos() + bDirection * collider->Height() * 0.5f;
